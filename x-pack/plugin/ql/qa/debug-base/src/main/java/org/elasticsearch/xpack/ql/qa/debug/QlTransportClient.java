...
public class QlTransportClient extends AbstractClient {
    ...
    private static ClientTemplate clientTemplate(List<Class<? extends Plugin>> plugins) {
        //
        // Initial setup
        //
        ...
        try {
            ...
            ActionModule actionModule = new ActionModule(true, settings, null, settingsModule.getIndexScopedSettings(),
                    ...
                    pluginsService.filterPlugins(ActionPlugin.class), null, null, null, null, systemIndices);
            modules.add(actionModule);

            // previously Node.createCircuitBreakerService(settingsModule.getSettings(), settingsModule.getClusterSettings())
    ...
    private static List<Class<? extends Plugin>> addMandatoryPlugins(List<Class<? extends Plugin>> plugins) {
        List<Class<? extends Plugin>> list = new ArrayList<>();
        list.add(Netty4Plugin.class);
        list.add(XPackPlugins.class);
        list.addAll(plugins);
        return list;
    }

    public QlTransportClient(List<Class<? extends Plugin>> plugins) {
        this(clientTemplate(addMandatoryPlugins(plugins)));
    }

    private QlTransportClient(ClientTemplate template) {
    ...
    public static class XPackPlugins extends Plugin implements ActionPlugin {
