...
public class QlHandler implements HttpHandler, AutoCloseable {

    protected static final Logger log = LogManager.getLogger(QlHandler.class.getName());

    private final RestHandler handler;
    private final QlNodeClient client;

    public QlHandler(QlNodeClient client, RestHandler restHandler) {
        this.client = client;
        ...
        try {
            ...
            if (!closeEndpoint) {
                if (channel.await()) {
                    ...
                    sendHttpResponse(http, new BytesRestResponse(channel, ExceptionsHelper.serverError("Timed-out")));
                }
            } else {
    ...
    private FakeRestRequest createRequest(HttpExchange http) throws IOException {
        ...
        XContentType contentType = XContentType.fromMediaType(
        ...
        if (indexOf > 0) {
            index = path.substring(1, indexOf);
