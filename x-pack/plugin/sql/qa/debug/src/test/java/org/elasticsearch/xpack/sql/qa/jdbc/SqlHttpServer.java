...
public class SqlHttpServer {
    ...
    private void initializeActions(HttpServer server) {
        // root action
        server.createContext("/", new SqlHandler(sqlClient, new RestMainAction()));
        server.createContext(Protocol.SQL_QUERY_REST_ENDPOINT, new SqlHandler(sqlClient, new RestSqlQueryAction()));
        server.createContext(Protocol.CLEAR_CURSOR_REST_ENDPOINT, new SqlHandler(sqlClient, new RestSqlClearCursorAction()));
    }
