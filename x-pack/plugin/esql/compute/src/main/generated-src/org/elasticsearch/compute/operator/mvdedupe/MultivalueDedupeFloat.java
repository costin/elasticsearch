 ...
 */
public class MultivalueDedupeFloat {
    /**
     ...
     * {@link #copyAndSort}
     */
    float[] work = new float[ArrayUtil.oversize(2, Float.BYTES)];
    /**
    ...
    public MultivalueDedupeFloat(FloatBlock block) {
        this.block = block;
    }
     ...
     */
    public FloatBlock dedupeToBlockAdaptive(BlockFactory blockFactory) {
        if (block.mvDeduplicated()) {
            block.incRef();
            return block;
        }
        try (FloatBlock.Builder builder = blockFactory.newFloatBlockBuilder(block.getPositionCount())) {
            for (int p = 0; p < block.getPositionCount(); p++) {
                int count = block.getValueCount(p);
                int first = block.getFirstValueIndex(p);
                switch (count) {
                    case 0 -> builder.appendNull();
                    case 1 -> builder.appendFloat(block.getFloat(first));
                    default -> {
                        /*
     ...
     */
    public FloatBlock dedupeToBlockUsingCopyAndSort(BlockFactory blockFactory) {
        if (block.mvDeduplicated()) {
            block.incRef();
            return block;
        }
     ...
     */
    public FloatBlock dedupeToBlockUsingCopyMissing(BlockFactory blockFactory) {
        if (block.mvDeduplicated()) {
            block.incRef();
            return block;
        }
     ...
     */
    public FloatBlock sortToBlock(BlockFactory blockFactory, boolean ascending) {
        try (FloatBlock.Builder builder = blockFactory.newFloatBlockBuilder(block.getPositionCount())) {
            for (int p = 0; p < block.getPositionCount(); p++) {
                int count = block.getValueCount(p);
                int first = block.getFirstValueIndex(p);
    ...
    public BatchEncoder batchEncoder(int batchSize) {
        block.incRef();
        return new BatchEncoder.Floats(batchSize) {
            @Override
    ...
    void copyMissing(int first, int count) {
        ...
        work[0] = block.getFloat(first);
        w = 1;
        i: for (int i = first + 1; i < end; i++) {
            float v = block.getFloat(i);
            for (int j = 0; j < w; j++) {
                if (v == work[j]) {
                    continue i;
                }
    ...
    /**
     * Writes an already deduplicated {@link #work} to a {@link FloatBlock.Builder}.
     */
    private void writeUniquedWork(FloatBlock.Builder builder) {
        if (w == 1) {
            builder.appendFloat(work[0]);
            return;
        }
        builder.beginPositionEntry();
        for (int i = 0; i < w; i++) {
            builder.appendFloat(work[i]);
        }
        builder.endPositionEntry();
    }

    /**
     * Writes a sorted {@link #work} to a {@link FloatBlock.Builder}, skipping duplicates.
     */
    private void deduplicatedSortedWork(FloatBlock.Builder builder) {
        builder.beginPositionEntry();
        float prev = work[0];
        builder.appendFloat(prev);
        for (int i = 1; i < w; i++) {
            if (prev != work[i]) {
                prev = work[i];
                builder.appendFloat(prev);
    ...
    /**
     * Writes a {@link #work} to a {@link FloatBlock.Builder}.
     */
    private void writeSortedWork(FloatBlock.Builder builder, boolean ascending) {
        builder.beginPositionEntry();
        for (int i = 0; i < w; i++) {
            if (ascending) {
                builder.appendFloat(work[i]);
            } else {
                builder.appendFloat(work[w - i - 1]);
            }
        }
        builder.endPositionEntry();
    }
    ...
    /**
     * Writes a deduplicated {@link #work} to a {@link BatchEncoder.Floats}.
     */
    private void encodeUniquedWork(BatchEncoder.Floats encoder) {
        for (int i = 0; i < w; i++) {
    ...
    private void hashAdd(IntBlock.Builder builder, LongHash hash, float v) {
        appendFound(builder, hash.add(v));
    }

    private long hashLookup(LongHash hash, float v) {
        return hash.find(v);
    }

    private void hashLookupSingle(IntBlock.Builder builder, LongHash hash, float v) {
    ...
    private static boolean valuesEqual(float lhs, float rhs) {
        return lhs == rhs;
    }
}
